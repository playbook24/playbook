/**
 * calendar.js
 * Gère l'affichage du calendrier, la logique des événements, la RÉPÉTITION et le VIEWER.
 */
...
window.ORB.calendar = {
    ...
    cacheDOM: function() {
        ...
        // Apparence (Couleur)
        this.eventColorInput = document.getElementById('event-color'); // Input caché
    ...
    bindEvents: function() {
        ...
        // Gestion Couleur (Clic sur les ronds)
        ...
        try {
            ...
            if (plans.length === 0) {
                ...
                plans.reverse().forEach(plan => {
                    ...
                    item.innerHTML = `<div style="font-weight:bold;">${plan.name}</div><div style="font-size:0.8em; opacity:0.7;">${new Date(plan.createdAt).toLocaleDateString()} - ${plan.playbookIds.length} exos</div>`;
