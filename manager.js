...
function initManager() {
    ...
    function createPlaybookCard(playbook) {
        ...
        // NOUVEAU : Crée le HTML pour les tags en utilisant les noms
        let tagsHtml = '';
    ...
    libraryGrid.addEventListener('click', async (e) => {
        ...
        if (e.target.closest('.card-btn-load')) {
            try {
                ...
                if (playbookRecord) {
                    const loadEvent = new CustomEvent('loadPlaybook', { detail: playbookRecord.playbookData });
